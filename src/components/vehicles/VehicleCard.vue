<template>
  <view class="vehicle-card" @tap="selectable && emit('select')">
    ...
    <view v-if="payableFare !== undefined" class="price">{{ formatPayableFare(payableFare, payableFareCurrency) }}</view><text v-if="payableFare !== undefined" class="discount">已減 {{ format(200) }}</text>
