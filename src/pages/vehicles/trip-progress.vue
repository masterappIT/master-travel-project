<template>
  <view class="page trip-progress-page" :style="responsiveStyle">
    <view class="canvas">
      <HomeMap map-id="trip-progress-map" :latitude="mapLatitude" :longitude="mapLongitude" :markers="mapMarkers" :polyline="mapPolyline" :route-summary="routeSummary" :center-trigger="centerTrigger" :pickup-label="mapOriginLabel" :destination-label="mapDestinationLabel" :fit-padding="[72, 42, 480, 42]" route-point-callouts full-screen />
...
const loadTrip = async () => {
  ...
  try {
    ...
    if (nextTrip.status !== 'COMPLETED' && hasPageInNavigationHistory('/pages/trips/pending', '/pages/vehicles/trip-progress')) {
      transitioning = true
      stopPolling()
      return openCachedPage('/pages/trips/trips')
    }
    if (nextTrip.status !== 'COMPLETED') return
    transitioning = true
    stopPolling()
    return openCachedPage(`/pages/vehicles/trip-complete?id=${encodeURIComponent(tripId.value)}`)
  } catch (error) {
    transitioning = false
