<template>
  <view class="page" :style="responsiveStyle">
    ...
    <view v-if="detailPriceOpen" class="detail-price-page">
      ...
      <view v-if="!detailLines.length" class="detail-price-row vehicle"><text>報價</text><text class="detail-price-value">暫無報價</text></view>
      <view class="detail-price-line first"></view>
      <text class="detail-price-service">用車服務費（包含高速費、停車費、路橋費）</text>
...
import { consumeFareQuote, planDrivingRoute } from '../../services/api'
...
const detailRowTop = (line: { type: string }, index: number) => {
  if (line.type === 'VEHICLE') return 309
  if (line.type === 'DISCOUNT') return 350 + Math.max(0, index - 1) * 41
  return 350 + Math.max(0, index - 1) * 41
}
