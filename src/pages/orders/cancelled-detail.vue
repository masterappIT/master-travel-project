  ...
  <view :class="['page', { 'has-load-error': loadError }]" :style="responsiveStyle">
    ...
    <template v-else-if="!loadError">
      ...
      <view :class="['payment', { completed: isCompleted, cancelled: isCancelled, pending: !isCompleted && !isCancelled }]">
        ...
        <view v-else-if="isCancelled && refundStatus" class="cancelled-tag">{{ refundStatus }}</view>
        <view v-else-if="isCompleted" class="paid-tag">已付款</view>
      </view>
...
const refundStatus = computed(() => {
  const payment = storedOrder.value?.payment
  if (!isCancelled.value || !payment) return ''
  return payment.status === 'REFUNDED' || payment.refundedAt ? '已退款' : '退款申請中'
})
...
.standard-detail-card .payment.cancelled { position: absolute; top: 15px; right: 25px; left: auto; width: 0; min-height: 0; height: 0; }
.standard-detail-card .payment.cancelled .cancelled-tag { position: absolute; top: 0; right: 0; }
