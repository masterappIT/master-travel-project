<template>
  <view class="page" :style="responsiveStyle">
    ...
    <template v-else>
    <view :class="['header', { 'traveling-header': isTraveling }]">
...
const selectedVehicle = computed(() => tripStore.chosenVehicle || {
  title: '高級跨境商務車',
  seats: 7,
  price: storedOrder.value?.payment?.total || 0
})
const vehicleLabel = computed(() => `${selectedVehicle.value.title}（${selectedVehicle.value.seats}座）`)
const amountLabel = computed(() => formatCurrencyAmount(storedOrder.value?.payment?.total || selectedVehicle.value.price, normalizeCurrency(storedOrder.value?.payment?.currency) || 'RMB'))
const paymentTotal = computed(() => storedOrder.value?.payment?.total || selectedVehicle.value.price)
const currencyLabel = computed(() => formatCurrencyAmount(0, normalizeCurrency(storedOrder.value?.payment?.currency) || 'RMB').replace('0.00', ''))
