<template>
  <view class="page" :style="responsiveStyle">
    ...
    <template v-else>
      ...
      <view class="traveling-card standard-detail-card pending-card">
...
const wallet = reactive({ fare: 0, withdrawable: 0 })
...
import { cancelClientTrip, getClientTrip, getFareQuote, payTrip, type ClientTrip, type FareQuote } from '../../services/api'
...
const loadOrder = async (url = '') => {
  ...
  try {
    ...
    else if (storedOrder.value.status === 'CONFIRMED') uni.redirectTo({ url: `/pages/orders/traveling-detail?id=${encodeURIComponent(storedOrder.value.id)}` })
