  ...
  <view class="page" :style="responsiveStyle">
    ...
    <view :class="['trip-card', { accepted: hasDriver }]">
      ...
      <view v-else class="accepted-driver">
        ...
        <view class="accepted-plate"><text>SH 8520</text><text>{{ acceptedVehiclePlate }}</text><view><text>粵</text><text>Z</text><image src="/static/vehicles/trip-waiting/plate-dot.svg" mode="aspectFit" /><text>XXX港</text></view></view>
      </view>
...
const tripId = ref('')
const previewAssigned = ref(false)
...
const acceptedVehiclePlate = computed(() => trip.value?.driver?.vehiclePlate || '—')
...
const loadTrip = async () => {
  ...
  try {
    ...
    trip.value = previewAssigned.value || previewProgress.value
      ? {
          ...loadedTrip,
          executionPhase: previewProgress.value ? 'IN_PROGRESS' : 'DRIVER_ASSIGNED',
          acceptedAt: loadedTrip.acceptedAt || new Date().toISOString(),
          driver: loadedTrip.driver || { name: '陳師傅', phone: '+852 91234567', vehiclePlate: 'HK-85-20' },
          vehicle: loadedTrip.vehicle || { id: 'preview', categoryId: null, categoryName: '高級跨境商務車', brand: 'Toyota', model: 'Alphard', series: 'Executive Lounge', seats: 6, modelChoiceLabel: 'Toyota Alphard' },
        }
      : loadedTrip
    refreshCountdown()
  } catch (error) {
...
onLoad(options => {
  ...
  const preview = options.preview === 'assigned' || options.preview === 'progress' ? `&preview=${options.preview}` : ''
  activatePage(`?id=${encodeURIComponent(options.id)}${preview}`)
})
// #ifdef H5
