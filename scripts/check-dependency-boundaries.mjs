...
for (const file of files) {
  ...
  for (const forbidden of config.forbiddenSourceReferences[area] ?? []) {
    ...
    const importsForbiddenArea = imports.some(specifier => {
      const importedPath = specifier.startsWith('.')
        ? path.normalize(path.join(path.dirname(file), specifier))
        : path.normalize(specifier)
      return importedPath === normalizedForbidden.slice(0, -1)
        || importedPath.startsWith(normalizedForbidden)
    })
    if (importsForbiddenArea) failures.push(`${file} imports forbidden area ${forbidden}`)
  }
}
