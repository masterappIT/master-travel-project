...
const messages = {
  en: {
    ...
    dashboard: 'Dashboard', users: 'Users', trips: 'Trips', charters: 'Business charter', addresses: 'Recommended addresses', membership: 'Membership plans', administrators: 'Administrators', auditLogs: 'Operation logs', adminConsole: 'ADMIN CONSOLE',
  ...
  zh: {
    ...
    dashboard: '仪表盘', users: '用户', trips: '行程', charters: '商务包车', addresses: '推荐地址', administrators: '管理员账户', auditLogs: '操作日志', adminConsole: '管理控制台', refresh: '刷新',
...
async function load() {
  ...
  try {
    ...
    if (view.value === 'dashboard') dashboard.value = await api('/admin/dashboard')
    if (['users', 'trips', 'charters'].includes(view.value)) users.value = (await api('/admin/users')).data
    if (view.value === 'trips') trips.value = (await api('/admin/trips')).data
    if (view.value === 'charters') charterOrders.value = (await api('/admin/charter-orders')).data
    if (view.value === 'addresses') {
    ...
    if (view.value === 'membership') membershipPlans.value = (await api('/admin/membership-plans')).data
    if (view.value === 'administrators') administrators.value = (await api('/admin/administrators')).data
    if (view.value === 'auditLogs') auditLogs.value = (await api('/admin/audit-logs')).data
    if (view.value === 'vehicles') { categories.value = (await api('/admin/vehicle-categories')).data; vehicles.value = (await api('/admin/vehicles')).data; extras.value = (await api('/admin/vehicle-extras')).data; distancePricing.value = (await api('/admin/distance-pricing')).data; pricingCurrency.value = distancePricing.value[0]?.currency === 'HKD$' ? 'HKD' : 'RMB' }
  }
...
function editUser(item) { userForm.value = { ...item } }
function resetUser() { userForm.value = { id: '', name: '', countryCode: '+852', phoneNumber: '' } }
...
function editTrip(item) { tripForm.value = { ...item, scheduledAt: dateTimeInput(item.scheduledAt) } }
...
async function saveTrip() { try { await api(`/admin/trips/${tripForm.value.id}`, { method: 'POST', body: JSON.stringify(tripForm.value) }); tripForm.value = null; await load() } catch (e) { error.value = displayError(e.message) } }
...
const formatBenefits = item => item.benefits.join(' · ')
function editExtra(item) { extraForm.value = { ...item } }
function resetExtra() { extraForm.value = { id: '', label: '', price: 0, currency: 'RMB¥', enabled: true, order: extras.value.length + 1 } }
async function saveExtra() { try { await api('/admin/vehicle-extras', { method: 'POST', body: JSON.stringify(extraForm.value) }); extraForm.value = null; await load() } catch (e) { error.value = displayError(e.message) } }
