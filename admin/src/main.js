...
const notificationRecipientSearch = ref('')
const driverRaceSaving = ref(false)
...
const tripPageSize = 10
watch([tripSearchQuery, tripStatusFilter, tripDateFilter], () => { tripPage.value = 1 }, { flush: 'sync' })
const tripDateYear = ref('')
const tripDateMonth = ref('')
const tripDateDay = ref('')
const tripDateYears = computed(() => { const year = new Date().getFullYear(); return Array.from({ length: 11 }, (_, index) => String(year - 5 + index)) })
const tripDateDays = computed(() => { const year = Number(tripDateYear.value) || new Date().getFullYear(); const month = Number(tripDateMonth.value) || 1; return Array.from({ length: new Date(year, month, 0).getDate() }, (_, index) => String(index + 1).padStart(2, '0')) })
watch([tripDateYear, tripDateMonth, tripDateDay], () => {
  tripDateFilter.value = tripDateYear.value && tripDateMonth.value && tripDateDay.value
    ? `${tripDateYear.value}-${tripDateMonth.value}-${tripDateDay.value}`
    : ''
})
const clearTripDateFilter = () => { tripDateYear.value = ''; tripDateMonth.value = ''; tripDateDay.value = ''; tripDateFilter.value = '' }
...
const drivers = ref(readStoredList('admin_drivers').filter(item => item && (item.affiliation || item.plateType || item.vehicleCategory || item.reviewStatus)))
const personnelForm = ref(null)
const driverForm = ref(null)
const selectedDriver = ref(null)
const settlementForm = ref(null)
const driverFilter = ref('全部')
const driverTypeFilter = ref('全部')
...
async function load() {
  ...
  try {
    ...
    if (requestedView === 'drivers') {
      const [categoryResult, driverResult] = await Promise.all([api('/admin/vehicle-categories'), api('/admin/drivers')])
      ...
      if (!driverResult.data.length && drivers.value.length) {
        for (const driver of drivers.value) await api('/admin/drivers', { method: 'POST', body: JSON.stringify(driver) })
        drivers.value = (await api('/admin/drivers')).data
      } else {
    ...
    if (requestedView === 'dispatch') {
      const [tripResult, driverResult] = await Promise.all([api('/admin/trips'), api('/admin/drivers')])
      trips.value = tripResult.data
      drivers.value = driverResult.data
      orderUrls.value = []
      for (const trip of trips.value) {
        const result = await api(`/admin/trips/${trip.id}/order-urls`)
        orderUrls.value.push(...result.data)
      }
      tripPage.value = 1
    }
    if (requestedView === 'trips') {
      const [tripResult, catalogResult, driverResult] = await Promise.all([api('/admin/trips'), api('/vehicles'), api('/admin/drivers')])
      trips.value = tripResult.data
      drivers.value = driverResult.data
      tripPage.value = 1
...
function resetNotification(template = 'order') {
  const templates = {
    order: { title: '您的跨境出行訂單已確認', content: '您的行程：香港 - 深圳機場（訂單編號：282678634）', audience: 'ALL_USERS', userIds: [], driverIds: [] },
    payment: { title: '您的餘額增值已到帳', content: '您的錢包餘額$0.00', audience: 'ALL_USERS', userIds: [], driverIds: [] },
    promotion: { title: '您的餘額提現已到帳', content: '您的錢包餘額$0.00', audience: 'ALL_USERS', userIds: [], driverIds: [] },
    system: { title: '您的訂單退款已到帳', content: '行程：香港 - 深圳機場（訂單編號：282678634）', audience: 'ALL_USERS', userIds: [], driverIds: [] }
  }
  notificationForm.value = { ...templates[template] }
  notificationRecipientSearch.value = ''
}
function clearNotificationRecipients() {
  if (!notificationForm.value) return
  notificationForm.value.userIds = []
  notificationForm.value.driverIds = []
}
function toggleNotificationRecipient(type, id, checked) {
  if (!notificationForm.value) return
  const key = type === 'user' ? 'userIds' : 'driverIds'
  const ids = new Set(notificationForm.value[key] || [])
  if (checked) ids.add(id)
  else ids.delete(id)
  notificationForm.value[key] = [...ids]
}
function notificationRecipientChecked(type, id) {
  const key = type === 'user' ? 'userIds' : 'driverIds'
  return Boolean(notificationForm.value?.[key]?.includes(id))
}
async function saveNotification() {
  if (!notificationForm.value) return
  try {
    await api('/admin/notifications', { method: 'POST', body: JSON.stringify(notificationForm.value) })
    notificationForm.value = null
    await load()
  } catch (e) { error.value = displayError(e.message) }
}
async function savePaymentSettings() {
  try {
    ...
    await api('/settings', {
      method: 'POST',
      body: JSON.stringify({
        driverRaceEnabled: paymentSettings.value.driverRaceEnabled,
...
async function updateUserStatus(item) {
  ...
  if (!confirm(nextEnabled ? `確定恢復用戶「${item.name || item.phone}」？` : `確定停用用戶「${item.name || item.phone}」？停用後現有登入會立即失效。`)) return
  try {
    const updated = await api(`/admin/users/${item.id}/status`, { method: 'POST', body: JSON.stringify({ enabled: nextEnabled }) })
...
async function saveDispatch() {
  if (!dispatchForm.value?.tripId || !dispatchForm.value.driverId) return
  try {
    await api(`/admin/trips/${dispatchForm.value.tripId}/dispatch`, { method: 'POST', body: JSON.stringify({ driverId: dispatchForm.value.driverId }) })
    dispatchForm.value = null
    await load()
  } catch (e) { error.value = displayError(e.message) }
}
function openOrderUrlForm(item) {
  const now = new Date()
  const later = new Date(now.getTime() + 24 * 60 * 60 * 1000)
  orderUrlForm.value = { tripId: item.id, driverId: item.driverId || '', validFrom: dateTimeInput(now.toISOString()), validUntil: dateTimeInput(later.toISOString()) }
}
const createOrderUrl = async () => {
  ...
  try {
    const result = await api(`/admin/trips/${orderUrlForm.value.tripId}/order-url`, { method: 'POST', body: JSON.stringify(orderUrlForm.value) })
...
async function copyOrderUrl() {
  if (!createdOrderUrl.value) return
  try { await navigator.clipboard.writeText(createdOrderUrl.value) } catch { error.value = '複製 URL 失敗，請手動複製' }
}
async function revokeOrderUrl(item) {
  if (!confirm('確定要撤銷此訂單 URL？')) return
  try { await api(`/admin/trips/${item.tripId}/order-urls/${item.id}/revoke`, { method: 'POST' }); await load() } catch (e) { error.value = displayError(e.message) }
}
...
function resetDriver() { driverForm.value = { id: '', driverType: '內部司機', name: '', affiliation: '香港', plateType: '單牌', hkPlate: '', mainlandPlate: '', phoneCountryCode: '+852', phone: '', vehicleCategory: '', vehicleColor: '', vehiclePhotos: [], reviewStatus: '待審核' } }
function editDriver(item) { driverForm.value = { driverType: '內部司機', affiliation: '香港', plateType: '單牌', phoneCountryCode: '+852', vehiclePhotos: [], reviewStatus: '待審核', ...item, vehiclePhotos: Array.isArray(item.vehiclePhotos) ? item.vehiclePhotos : item.vehiclePhotos ? [item.vehiclePhotos] : [] } }
async function uploadDriverPhotos(event) {
  ...
  try {
    ...
    error.value = ''
  } catch (e) { error.value = '車輛相片上傳失敗' }
}
function removeDriverPhoto(index) { driverForm.value.vehiclePhotos = (driverForm.value.vehiclePhotos || []).filter((_, photoIndex) => photoIndex !== index) }
async function saveDriver() {
  ...
  try {
    await api('/admin/drivers', { method: 'POST', body: JSON.stringify(form) })
    driverForm.value = null; error.value = ''; await load()
  } catch (e) { error.value = displayError(e.message) }
}
async function removeDriver(item) { if (confirm(`刪除司機「${item.name}」？`)) { try { await api(`/admin/drivers/${item.id}`, { method: 'DELETE' }); await load() } catch (e) { error.value = displayError(e.message) } } }
...
function resetSettlement(item = selectedDriver.value) { if (item) settlementForm.value = { driverId: item.id, settlementMethod: item.settlementMethod || '月結', settlementAccount: item.settlementAccount || '' } }
async function saveSettlement() { if (!settlementForm.value) return; try { const driver = drivers.value.find(item => item.id === settlementForm.value.driverId); if (!driver) return; await api('/admin/drivers', { method: 'POST', body: JSON.stringify({ ...driver, ...settlementForm.value }) }); settlementForm.value = null; await load() } catch (e) { error.value = displayError(e.message) } }
...
const filteredDrivers = computed(() => {
  ...
  return drivers.value.filter(item => {
    const matchesStatus = driverFilter.value === '全部' || (item.reviewStatus || '待審核') === driverFilter.value
    const matchesType = driverTypeFilter.value === '全部' || (item.driverType || '內部司機') === driverTypeFilter.value
    const matchesQuery = !query || [item.name, item.phone, item.hkPlate, item.mainlandPlate, item.vehicleCategory].some(value => String(value || '').toLowerCase().includes(query))
...
const filteredNotificationUsers = computed(() => {
  const query = notificationRecipientSearch.value.trim().toLowerCase()
  if (!query) return notificationUsers.value
  return notificationUsers.value.filter(item => `${item.displayName || ''} ${item.name || ''} ${item.phoneNumber || ''} ${item.id}`.toLowerCase().includes(query))
})
const filteredNotificationDrivers = computed(() => {
  const query = notificationRecipientSearch.value.trim().toLowerCase()
  if (!query) return notificationDrivers.value
  return notificationDrivers.value.filter(item => `${item.name || ''} ${item.phone || ''} ${item.id}`.toLowerCase().includes(query))
})

const filteredUsers = computed(() => {
  const query = userSearchQuery.value.trim().toLowerCase()
  return users.value.filter(item => {
    const matchesQuery = !query || [item.name, item.phoneNumber, item.id].some(value => String(value || '').toLowerCase().includes(query))
    const matchesStatus = userStatusFilter.value === 'ALL' || (userStatusFilter.value === 'ENABLED' ? item.enabled !== false : item.enabled === false)
...
watch([userSearchQuery, userStatusFilter], () => { userPage.value = 1 })
watch(userPageCount, count => { if (userPage.value > count) userPage.value = count })
const goToUserPage = page => { userPage.value = Math.min(Math.max(1, page), userPageCount.value) }
const filteredTrips = computed(() => {
  ...
  return trips.value.filter(item => {
    ...
    const matchesQuery = !query || [item.id, displayId, item.origin, item.destination, item.user?.name, item.user?.phoneNumber].some(value => String(value || '').toLowerCase().includes(query))
...
const dispatchFilteredTrips = computed(() => {
  const query = dispatchSearch.value.trim().toLowerCase()
  return trips.value.filter(item => !query || [item.id, item.origin, item.destination].join(' ').toLowerCase().includes(query))
})
const dispatchPageCount = computed(() => Math.max(1, Math.ceil(dispatchFilteredTrips.value.length / dispatchPageSize)))
const pagedDispatchTrips = computed(() => dispatchFilteredTrips.value.slice((dispatchPage.value - 1) * dispatchPageSize, dispatchPage.value * dispatchPageSize))
...
const goToDispatchPage = page => { dispatchPage.value = Math.min(Math.max(1, page), dispatchPageCount.value) }
const tripPageCount = computed(() => Math.max(1, Math.ceil(filteredTrips.value.length / tripPageSize)))
const pagedTrips = computed(() => {
  const start = (tripPage.value - 1) * tripPageSize
  return filteredTrips.value.slice(start, start + tripPageSize)
})
watch(tripPageCount, (count) => {
  if (tripPage.value > count) tripPage.value = count
})
const goToTripPage = (page) => { tripPage.value = Math.min(Math.max(1, page), tripPageCount.value) }
