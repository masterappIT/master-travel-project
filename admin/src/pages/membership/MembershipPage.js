...
export const MembershipPage = {
  ...
  template: String.raw`<section v-if="view==='membership'" class="editor-section membership-management">
    ...
    <div class="panel membership-order-panel">
      ...
      <div v-if="membershipOrderTotal" class="pagination-controls" aria-label="會員訂單分頁"><button type="button" class="secondary" :disabled="membershipOrderPage === 1" @click="goToMembershipOrderPage(membershipOrderPage - 1)">上一頁</button><span>第 {{membershipOrderPage}} / {{membershipOrderPageCount}} 頁 · 共 {{membershipOrderTotal}} 筆</span><button type="button" class="secondary" :disabled="membershipOrderPage === membershipOrderPageCount" @click="goToMembershipOrderPage(membershipOrderPage + 1)">下一頁</button></div>
    </div>
    <div v-if="membershipForm" class="modal-backdrop" @click.self="membershipForm=null"><form class="record-form membership-form" @submit.prevent="saveMembership">
      ...
      <div class="membership-form-body">
        ...
        <div class="membership-form-section"><h4>權益與顯示</h4><label class="membership-benefits-field">會員權益<textarea v-model="membershipForm.benefits" rows="4" placeholder="每行輸入一項會員權益"></textarea><small>每行一項，將依輸入順序顯示。</small></label><div class="membership-form-options"><label>顯示排序<input v-model.number="membershipForm.order" type="number" min="1"/></label><label class="membership-toggle"><AdminCheckbox v-model="membershipForm.recommended" type="checkbox"/><span><strong>推薦方案</strong><small>在會員端加上推薦標示</small></span></label><label class="membership-toggle"><AdminCheckbox v-model="membershipForm.enabled" type="checkbox"/><span><strong>啟用方案</strong><small>允許會員選擇此方案</small></span></label></div></div>
