export function createNotificationsActions({ api, notificationForm, notificationRecipientSearch, load, error, displayError }) {
  function resetNotification(template = 'order') {
    const templates = { order: { title: '您的跨境出行訂單已確認', content: '您的行程：香港 - 深圳機場（訂單編號：282678634）', audience: 'ALL_USERS', userIds: [], driverIds: [] }, payment: { title: '您的餘額增值已到帳', content: '您的錢包餘額$0.00', audience: 'ALL_USERS', userIds: [], driverIds: [] }, promotion: { title: '您的餘額提現已到帳', content: '您的錢包餘額$0.00', audience: 'ALL_USERS', userIds: [], driverIds: [] }, system: { title: '您的訂單退款已到帳', content: '行程：香港 - 深圳機場（訂單編號：282678634）', audience: 'ALL_USERS', userIds: [], driverIds: [] } }
    notificationForm.value = { ...templates[template] }; notificationRecipientSearch.value = ''
  }
  ...
  return { resetNotification, clearNotificationRecipients, toggleNotificationRecipient, notificationRecipientChecked, saveNotification }
}
