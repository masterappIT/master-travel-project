  ...
  template: String.raw`<section v-if="view==='notifications'" class="editor-section notifications-page">
    ...
    <div class="notification-workspace">
      <div class="panel notification-composer-panel">
        <div class="notification-panel-heading"><div><span class="panel-kicker">CREATE</span><h3>建立推送</h3><p>選擇模板後編輯消息內容，送出前可先查看預覽。</p></div><select aria-label="消息模板" @change="resetNotification($event.target.value)"><option v-for="template in notificationTemplates" :key="template.id" :value="template.type">{{template.name}}</option></select></div>
        <form v-if="notificationForm" class="record-form notification-form" @submit.prevent="saveNotification">
          ...
          <div class="notification-form-actions"><label class="notification-field"><span>重要消息</span><input v-model="notificationForm.important" type="checkbox"/></label><button type="submit" class="notification-send-action">發送消息</button><button type="button" class="secondary" @click="notificationForm=null">取消</button></div>
        </form>
      </div>
    ...
    <div class="panel notification-template-panel"><div class="notification-history-heading"><div><span class="panel-kicker">TEMPLATE LIBRARY</span><h3>推送模板</h3><p>保留內建模板，也可新增自訂發送模板。</p></div><button v-if="canWrite" type="button" class="secondary" @click="notificationTemplateForm = { name: '', type: 'custom', title: '', content: '', audience: 'ALL_USERS', important: false, enabled: true }">＋ 新增模板</button></div><div class="notification-template-list"><div v-for="template in notificationTemplates" :key="template.id" class="notification-template-item"><div><strong>{{template.name}}</strong><span>{{template.type}} · {{template.builtIn ? '內建模板' : '自訂模板'}}</span></div><button type="button" class="secondary" @click="resetNotification(template.type)">套用</button></div></div><form v-if="notificationTemplateForm" class="record-form notification-form" @submit.prevent="createTemplate(notificationTemplateForm).then(() => notificationTemplateForm = null)"><label class="notification-field"><span>模板名稱</span><input v-model="notificationTemplateForm.name" required /></label><label class="notification-field"><span>模板類型</span><input v-model="notificationTemplateForm.type" required /></label><label class="notification-field"><span>標題模板</span><input v-model="notificationTemplateForm.title" required /></label><label class="notification-field"><span>內容模板</span><textarea v-model="notificationTemplateForm.content" rows="3" required></textarea></label><div class="notification-form-actions"><button type="submit" class="notification-send-action">保存模板</button><button type="button" class="secondary" @click="notificationTemplateForm = null">取消</button></div></form></div>
