  ...
  template: String.raw`
    <section v-if="view==='driver-vehicles'" class="vehicle-registry">
      ...
      <div v-if="assignmentVehicle" class="modal-backdrop" @click.self="closeVehicleAssignments"><section class="vehicle-assignment-modal" role="dialog" aria-modal="true" aria-labelledby="vehicle-assignment-title">
        ...
        <div class="vehicle-assignment-body"><div class="vehicle-assignment-list"><div v-for="assignment in vehicleAssignments" :key="assignment.id" class="vehicle-assignment-row"><div class="vehicle-driver-avatar">{{assignment.driver.name?.slice(0,1) || '司'}}</div><div><strong>{{assignment.driver.name}}</strong><span>{{assignment.driver.phone || '未提供電話'}}{{assignment.driver.enabled === false ? ' · 帳號已停用' : ''}}</span></div><button type="button" v-if="canWrite" class="vehicle-unbind-action" @click="unbindVehicleDriver(assignment.driver.id)">解除</button></div><div v-if="!vehicleAssignments.length" class="vehicle-inline-empty">目前沒有綁定司機</div></div>
      ...
      <form v-if="vehicleForm" class="vehicle-form-drawer" @submit.prevent="saveVehicle" aria-labelledby="vehicle-form-title">
        ...
        <div class="vehicle-form-body">
          <section class="vehicle-form-section"><div class="vehicle-section-heading"><span>1</span><div><h3>基本資料</h3><p>設定車輛歸屬、類別及外觀。</p></div></div><div class="vehicle-form-grid">
            ...
            <label><span>車牌類型</span><select v-model="vehicleForm.plateType" :disabled="vehicleForm.vehicleOwnership==='中國內地'"><option>單牌</option><option>兩地牌</option></select><small v-if="vehicleForm.vehicleOwnership==='中國內地'">內地車輛固定使用兩地牌</small></label>
          ...
          <section class="vehicle-form-section"><div class="vehicle-section-heading"><span>2</span><div><h3>車牌資料</h3><p>只顯示目前歸屬地與車牌類型需要的欄位。</p></div></div><div class="vehicle-form-grid">
            <label v-if="vehicleForm.vehicleOwnership==='香港'"><span>香港車牌</span><input v-model="vehicleForm.hkPlate" autocomplete="off" placeholder="AB 1234"/><small>英文字母、數字及空格，最多 8 個字元</small></label>
            <label v-if="vehicleForm.vehicleOwnership==='澳門'"><span>澳門車牌</span><input v-model="vehicleForm.macauPlate" autocomplete="off" placeholder="AA-00-00"/><small>儲存時會自動整理為 AA-00-00</small></label>
            <label v-if="vehicleForm.vehicleOwnership==='中國內地' || vehicleForm.plateType==='兩地牌'" class="vehicle-mainland-field"><span>內地／跨境車牌</span><div class="vehicle-composed-plate"><span v-if="vehicleForm.vehicleOwnership==='香港'">粵Z·</span><span v-else-if="vehicleForm.vehicleOwnership==='澳門'">粵Z·</span><span v-else>粵</span><input v-model="vehicleForm.mainlandPlate" autocomplete="off" :placeholder="vehicleForm.vehicleOwnership==='中國內地' ? 'A·12345' : '1234'"/><span v-if="vehicleForm.vehicleOwnership==='香港'">港</span><span v-else-if="vehicleForm.vehicleOwnership==='澳門'">澳</span></div><small>固定地區字首與尾碼由系統自動加入</small></label>
          </div></section>
          <section class="vehicle-form-section"><div class="vehicle-section-heading"><span>3</span><div><h3>司機與相片</h3><p>建立時可先綁定一名司機，也可稍後管理多人綁定。</p></div></div><div class="vehicle-form-grid">
            <label><span>初始綁定司機 <em>選填</em></span><select v-model="vehicleForm.driverId"><option value="">暫不綁定</option><option v-for="driver in drivers" :key="driver.id" :value="driver.id">{{driver.name}} · {{driver.phone || '未提供電話'}}</option></select></label>
